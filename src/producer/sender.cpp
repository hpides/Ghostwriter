...
void Sender::Send(Batch *batch, uint64_t (&latencies)[4]) {
  ...
  latencies[0] = std::chrono::duration_cast<std::chrono::microseconds>(
  ...
  latencies[1] =
      std::chrono::duration_cast<std::chrono::microseconds>(staged - start)
          .count();
  latencies[2] =
      std::chrono::duration_cast<std::chrono::microseconds>(stored - staged)
          .count();
  latencies[3] =
      std::chrono::duration_cast<std::chrono::microseconds>(committed - stored)
          .count();
}
...
void Sender::Store(Batch *batch, uint64_t offset) {
  ...
  ucs_status_ptr_t status_ptr =
      endpoint.put(batch->getBuffer(), batch->getSize(),
                   endpoint.GetRemoteAddress() + offset, empty_cb);
...
std::pair<uint64_t, uint64_t> Sender::Stage(Batch *batch) {
  if (batch_ < 1) {
    std::unique_ptr<Message> stage_message = message_generator_p_->StageRequest(
        batch->getTopic(), batch->getPartition(), batch->getSize(), UINT64_MAX);
    UCP::Endpoint &endpoint = connection_manager_p_->GetConnection(
        config_.broker_node_ip, config_.broker_node_port);
    SendMessage(*stage_message, endpoint);
    std::unique_ptr<char> buffer = Client::ReceiveMessage(endpoint);
    auto base_message =
        flatbuffers::GetRoot<Rembrandt::Protocol::BaseMessage>(buffer.get());
    auto union_type = base_message->content_type();
    switch (union_type) {
    case Rembrandt::Protocol::Message_StageResponse: {
      auto staged = static_cast<const Rembrandt::Protocol::StageResponse *>(
...
bool Sender::Commit(uint32_t topic_id, uint32_t partition_id,
                    uint64_t logical_offset, uint64_t message_size) {
  std::unique_ptr<Message> commit_message = message_generator_p_->CommitRequest(
      topic_id, partition_id, logical_offset, message_size);
  UCP::Endpoint &endpoint = connection_manager_p_->GetConnection(
      config_.broker_node_ip, config_.broker_node_port);
