...
WindowedLatencyLogger::WindowedLatencyLogger(long batch_count, long window_size) : active_(false),
                                                                                   counter_(0l),
...
void WindowedLatencyLogger::Log(long latency) {
  if (active_) {
...
void WindowedLatencyLogger::Output(std::string dir, std::string prefix) {
  OutputTimeline(dir, prefix);
  OutputHistogram(dir, prefix);
}
void WindowedLatencyLogger::OutputHistogram(std::string dir, std::string prefix) {
  std::filesystem::path
...
void WindowedLatencyLogger::OutputTimeline(std::string dir, std::string prefix) {
