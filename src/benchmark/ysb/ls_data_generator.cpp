...
void generate(record& data, size_t campaign_offset, uint64_t campaign_lsb, uint64_t campaign_msb, size_t event_id)
{
  event_id = event_id % 3;

  memcpy(data.campaign_id, &campaign_msb, 8);

  uint64_t campaign_lsbr = campaign_lsb + campaign_offset;
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 5)
  {
    ...
    campaignCnt = atoi(argv[4]);
    cout << "using paramter filepath=" << filePath << " tupleCnt=" << tupleCnt << " threadCnt=" << threadCnt << " campaignCnt=" << campaignCnt << endl;
  }
  else
  {
  ...
  std::uniform_int_distribution<size_t> disi(0, campaignCnt);
