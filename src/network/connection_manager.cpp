...
UCP::Endpoint *ConnectionManager::FindConnection(const std::string &server_addr,
                                                 uint16_t port) const {
...
void ConnectionManager::Connect(const std::string &server_addr, uint16_t port) {
  std::unique_ptr<UCP::Endpoint> endpoint =
      endpoint_factory_p_->Create(worker_, server_addr, port);
...
void ConnectionManager::RegisterRemoteMemory(const std::string &server_addr,
  ...
  case Rembrandt::Protocol::Message_RMemInfoResponse: {
    ...
    auto rmem_info = static_cast<const Rembrandt::Protocol::RMemInfoResponse *>(
        base_message->content());
    endpoint.RegisterRMemInfo(rmem_info->remote_key()->str(),
                              rmem_info->remote_address());
    break;
  }
  default: {
...
void ConnectionManager::ReceiveInitializeResponse(
    UCP::Endpoint &endpoint) const {
  std::unique_ptr<char> buffer = ReceiveMessage(endpoint);
  auto base_message =
      flatbuffers::GetRoot<Rembrandt::Protocol::BaseMessage>(buffer.get());
  auto union_type = base_message->content_type();
...
void ConnectionManager::SendMessage(const Message &message,
                                    const UCP::Endpoint &endpoint) const {
...
std::unique_ptr<char>
ConnectionManager::ReceiveMessage(const UCP::Endpoint &endpoint) const {
