...
ConnectionManager::ConnectionManager(UCP::Worker &worker, UCP::EndpointFactory *endpoint_factory) :
...
void ConnectionManager::Connect(char *server_addr, uint16_t port) {
  connections_[std::pair(server_addr, port)] = endpoint_factory_->Create(worker_, server_addr, port);
}
